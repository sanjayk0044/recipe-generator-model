    ...
    for recipe_name in recipe_names:
        try:
            ...
            if results:
                ...
                image_urls[recipe_name] = image  # Append the image URL directly
            else:
                print(f"No images found in DuckDuckGo search for '{recipe_name}'")
                image_urls[recipe_name] = None  # Append None if no image found
        except Exception as e:
            print(f"An error occurred with DuckDuckGo search for '{recipe_name}': {e}")
            image_urls[recipe_name] = None  # Append None on exception
